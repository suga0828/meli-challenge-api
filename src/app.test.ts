...
const ID = 'id';
const MOCKED_ITEM = { id: ID, shipping: {}, seller_address: { state: {} } } as unknown as Item;

const Q = 'query';
const MOCKED_ITEMS = { results: [], filters: [{ id: 'category', values: [{ path_from_root: [] }] }] } as unknown as MeliProducts;
...
describe('app', () => {
  ...
  describe('/api/items/:id', () => {
    ...
    xtest('should return error on error', () => {
      jest.clearAllMocks();
      const ERROR = 'casita';
      const spy = jest.spyOn(axios, 'get').mockReturnValue(Promise.reject(ERROR));
  
      request(app).get(`/api/items/${ID}`).catch(error => {
